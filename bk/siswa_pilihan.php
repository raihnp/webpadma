...
$id=$_GET["id"];
$sis=mysqli_query($conn,"SELECT * FROM siswa WHERE id_siswa = '$id'");
$si = mysqli_fetch_assoc($sis);
$nis = $si["nis"];
...
<a href="exstra_db.php"><h3>Pilihan Exstra-Kurikuler</h3></a>
<?php $s=1; ?>
<?php
   foreach ($exstra as $key) {?>
<p><?= $s; ?> - <input type="checkbox" name="target[]" value="<?php echo $key; ?>"> <?php echo $key; ?>
   </p>
<?php $s++; ?>
    <?php } ?>
...
<a href="kelas.php" class="btn btn-default">Cancel</a>
  
    </form>

    <?php
    if(isset($_POST["simpan"])){
      $pilih = implode(",", $_POST['target']);

      $query = "INSERT INTO pilihan VALUES('','$nis','$pilih')";
