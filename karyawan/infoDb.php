...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM info WHERE tanggal LIKE '%$keyword%' OR  judul LIKE '%$keyword%' ORDER BY id_info DESC");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM info");
      $sql3 = $sql2;
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM info ORDER BY id_info DESC LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM info ");
...
$query = "UPDATE karyawan SET info = 'outline-primary' WHERE id_karyawan = '$id_karyawan'";
  ...
  <body>
   <div class="container">
    ...
    <div class="card p-1" style="width: 100%;">
      ...
      <span style="font-size: 20px">Informasi ..Pengumuman..</span>
      <span class="float-right">
        ...
        <?php while($row = mysqli_fetch_assoc($sql)) : ?>
          <tr>
            ...
            <td><?=$row["tanggal"]; ?></td>
            <td><?=$row["judul"]; ?></td>
            <td><?=$row["guru"]; ?> <?=$row["karyawan"]; ?> <?=$row["siswa"]; ?> <?=$row["ortu"]; ?></td>
