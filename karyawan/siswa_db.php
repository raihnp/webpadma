...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM siswa WHERE nis LIKE '%$keyword%' OR siswa LIKE '%$keyword%' OR kls LIKE  '%$keyword%' ORDER BY nis asc");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM siswa");
      $sql3 = mysqli_num_rows($sq);
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM siswa ORDER BY nis asc LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM siswa");
      $sql3 = mysqli_num_rows($sq);   
    }
    ...
    <tr>
      ...
      <td>
        ...
        if ($_SESSION["level"]!='4'){
          if ($_SESSION["level"]!='41'){
            if ($_SESSION["level"]!='42'){
            }else{;?>    <a href="perpusDb.php?id=<?=$row["id_siswa"];?>&k=<?=$k;?>&nis=<?=$row["nis"];?>"><?=$row["siswa"];?></a>    <?php }
          }else{;?>     <a href="tuDb.php?id=<?=$row["id_siswa"];?>&k=<?=$k;?>&nis=<?=$row["nis"];?>"><?=$row["siswa"];?></a>     <?php }
        }else{;?>     <?=$row["siswa"];?>      <?php }
        ;?>
      </td>
