...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM perpus WHERE nis = '$nis' AND tanggal LIKE '%$keyword%' OR  pesan LIKE '%$keyword%' ORDER BY id_perpus DESC");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM perpus");
      $sql3 = $sql2;
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM perpus WHERE nis = '$nis' ORDER BY id_perpus DESC LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM perpus ");
  ...
  <body>
   <div class="container">
    ...
    <div class="card p-1" style="width: 100%;">
      ...
      <span style="font-size: 20px">PerpustakaanDb</span>
      <span class="hidden float-right"><a href="perpus_add.php?id=<?=$id;?>&k=<?=$k;?>" class="btn btn-outline-info btn-sm"><img src="../img/add.png"></a></span>
