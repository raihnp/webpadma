...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM keluar ORDER BY id_keluar DESC LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM keluar ");
       ...
       <table class="table table-hover">
        ...
        <?php while($row = mysqli_fetch_assoc($sql)) : ?>
          <tr>
           ...
           <td class="text-center"><a href="doc_k_edit.php?id=<?=$row["id_keluar"]; ?>">O</a> | <a href="info_delete.php?id=<?=$row["id_info"]; ?>" onclick = "return confirm('yakin ?');">X</a></td>
          </tr>
            <?php $s++; ?>
        <?php endwhile; ?>
      </table>
