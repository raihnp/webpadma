...
$sql = mysqli_query($conn, "SELECT * FROM ijin WHERE done ='ok'"); 

$query1 = "UPDATE karyawan SET ijin = 'outline-success'";
  ...
  <body>
   <div class="container">
    ...
    <div class="card p-1" style="width: 100%;">
      ...
      <span style="font-size: 20px">Pengajuan Ijin Siswa</span>
      </div>
        ...
        <?php while($row = mysqli_fetch_assoc($sql)) : ?>
          <?php
          $nis = $row["nis"];
          $que = mysqli_query($conn, "SELECT * FROM siswa WHERE nis = '$nis'");
          $sis = mysqli_fetch_assoc($que);
          $nama = $sis["siswa"];
          ?>
          <tr>
            <td><?= $s; ?></td>
            <td><?=$row["dino"]; ?></td>
            <td><?=$nis; ?></td>
            <td><a href="ket_cetak.php?id=<?=$row["id_ijin"]; ?>&nis=<?=$nis;?>&ijin=<?=$row["ijin"]; ?>"><?=$nama;?></a></td>
            <td><?=$row["ijin"]; ?></td>
          </tr>
            <?php $s++; ?>
        <?php endwhile; ?>  
