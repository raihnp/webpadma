...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM masuk WHERE tanggal LIKE '%$keyword%' OR  des LIKE '%$keyword%' ORDER BY id_masuk DESC");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM masuk");
      $sql3 = $sql2;
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM masuk ORDER BY id_masuk DESC LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM masuk ");
...
<div class="col-md-10">
    <!-- ------------ --><div class="form p-1">
      ...
      <span style="font-size: 20px">Surat Masuk</span>
        ...
        <span class="hidden float-right"><a href="doc_m_add.php" class="btn btn-outline-info btn-sm"><img src="../img/add.png"></a></span> 
       ...
       <table class="table table-hover">
        ...
        <?php while($row = mysqli_fetch_assoc($sql)) : ?>
          <tr>
           ...
           <td class="text-center"><a href="doc_m_edit.php?id=<?=$row["id_masuk"]; ?>">O</a> | <a href="info_delete.php?id=<?=$row["id_info"]; ?>" onclick = "return confirm('yakin ?');">X</a></td>
          </tr>
            <?php $s++; ?>
        <?php endwhile; ?>
      </table>
