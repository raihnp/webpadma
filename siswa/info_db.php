...
$sql = mysqli_query($conn,"SELECT * FROM info WHERE siswa = 'siswa' ORDER BY id_info DESC");

$query = "UPDATE siswa SET info = 'outline-success' WHERE id_siswa = '$id_siswa'";
     ...
     <span style="font-size: 20px">Informasi ..Pengumuman..</span>
        ...
        <?php while($row = mysqli_fetch_assoc($sql)) : ?>
          <tr>
            ...
            <td><?=$row["tanggal"]; ?></td>
            <td><?=$row["judul"]; ?></td>
            <td class="hidden"><?=$row["guru"]; ?> <?=$row["karyawan"]; ?> <?=$row["siswa"]; ?> <?=$row["ortu"]; ?></td>
