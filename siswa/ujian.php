...
$sql = mysqli_query($conn, "SELECT * FROM kalender WHERE kls = '$k' ORDER BY date DESC");

$query = "UPDATE siswa SET ujian = 'outline-success' WHERE id_siswa = '$id_siswa'";
     ...
     <br>
        ...
        ?>
          <tr>
             <td><?= $s; ?></td>
            <td><?= $tgl; ?></td>
            <td><?=$row["description"]; ?><br><?= $row["user"] ?></td>
            <td><?=$row["keterangan"]; ?></td>
            <td><a href="link.php?id=<?=$row["id_kalender"]; ?>"><i class="material-icons"><?=$row["icons"]; ?></i></a></td>
          </tr>
        <?php 
        $s++;
        endwhile; ?>  
