...
$foto = $rowx["foto"];
$status = $rowx["status"];
$sql = mysqli_query($conn, "SELECT * FROM pass WHERE id_siswa = '$id_siswa'");
...
if(isset($_POST["submit"])){
  ...
  $nama = ($_POST["nama"]) ;
   $use = mysqli_query($conn, "SELECT * FROM pass WHERE user = '$nama'");
     ...
     echo"<script>alert ('nama user sudah terdaftar !');document.location.href = 'siswa_pass.php?id=$id';</script>"; 
     exit;}else {$user = $nama;}
  $password = ($_POST["password"]);
  $password2 = ($_POST["password2"]);
    if( $password !== $password2){
    echo"<script>alert('konfirmasi password gagal');document.location.href = 'siswa_pass.php?id=$id';</script>";exit;}
  $pass = password_hash($password, PASSWORD_DEFAULT);
  $query = "UPDATE pass SET user = '$user', pass = '$pass' WHERE id_siswa = '$id_siswa'";
  mysqli_query($conn, $query);
  if (mysqli_affected_rows($conn) > 0) {
    echo"<script>alert('Data berhasil dirubah');document.location.href = 'siswa_db.php';</script>";
    }else{echo"<script>alert('Data gagal dirubah');document.location.href = 'siswa_db.php';
    </script>";}
}
?>
<!doctype html>
...
<a href="siswa.php" class="btn btn-outline-dark">Cancel</a></p>
    </form>
  </div>
   <!-- -------------- -->
</div>
