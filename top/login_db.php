...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM login WHERE tanggal LIKE '%$keyword%' OR  nama LIKE '%$keyword%' ORDER BY id_login DESC");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM login");
      $sql3 = $sql2;
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM login ORDER BY id_login DESC LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM login ");
    ...
    <div class="card p-1" style="width: 100%;">
      ...
      <span class="float-right">
        <form action="" method="POST">
          ...
          <tr>
            <th>No</th>
            <th class="hidden">Kelompok</th>
            <th>Nama</th>
            <th class="hidden">Masuk</th>
            <th class="hidden">Keluar</th>
         </tr>
        <?php while($row = mysqli_fetch_assoc($sql)) : ?>
          <tr>
            <td><?= $no++; ?></td>
            <td class="hidden"><?=$row["kelompok"]; ?><br>Level : <?=$row["level"]; ?></td>
            <td><?=$row["nama"]; ?><br><i><?=$row["jab"]; ?></i></td>
            <td class="hidden"><?=$row["mulai"]; ?></td>
            <td class="hidden"><?=$row["keluar"]; ?></td>
          </tr>
        <?php endwhile; ?>  
        </table>
