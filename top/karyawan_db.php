...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM karyawan WHERE switch ='on' AND code LIKE '%$keyword%' OR karyawan LIKE '%$keyword%' OR jabatan LIKE  '%$keyword%' ORDER BY id_karyawan asc");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM karyawan WHERE switch ='on'");
      $sql3 = mysqli_num_rows($sq);
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM karyawan ORDER BY id_karyawan asc LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM karyawan WHERE switch ='on'");
      $sql3 = mysqli_num_rows($sq);   
    }
  ...
  <body>
   <div class="container">
    ...
    <div class="form p-1">
      ...
      <span style="font-size: 20px">Database Karyawan [<a href="karyawan_i_db.php">Inbox</a>]</span><span class="hidden float-right"><a href="karyawan_add.php" class="btn btn-outline-info btn-sm"><img src="../img/add.png"></a></span>
      <span class="float-right">
       ...
       <table class="table table-hover">
          ...
          <tr>
            <th><?=$no;?></th>
            <td class="d-inline-block align-top" style="text-align: center;"><img src="../img/karyawan/<?=$row["foto"];?>"width ="50px"><br><?=$row["code"];?></td>
            <td><?=$row["karyawan"];?><br><i><?=$row["jabatan"];?></i></td>
            ...
            <td class="na">
             ...
             <td class="na"><a href="karyawan_aktif.php?id=<?=$row["id_karyawan"]; ?>"><?=$row["status"]; ?></a><br><a href="karyawan_reset.php?id=<?=$row["id_karyawan"]; ?>" onclick = "return confirm('yakin ?');">Reset</a><br><a href="karyawan_level.php?id=<?=$row["id_karyawan"]; ?>">Level</a></td>
             <td class="na"><a href="karyawan_edit.php?id=<?=$row["id_karyawan"];?>">O</a> | <a href="karyawan_delete.php?id=<?=$row["id_karyawan"];?>"  onclick = "return confirm('yakin ?');">X</a></td>
          </tr>
        <?php $no++;?>
         <?php endwhile; ?>
      </table>
