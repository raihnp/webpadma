...
$foto = $rowx["foto"];
$id = $_GET["id"];
$sql = mysqli_query($conn, "SELECT * FROM guru WHERE switch = 'on' AND id_guru = '$id'");
$row=mysqli_fetch_assoc($sql);
if(isset($_POST["simpan"])){
  $status = $_POST["status"];
  $query = "UPDATE guru SET status = '$status' WHERE id_guru = '$id'";
  mysqli_query($conn, $query);
  if (mysqli_affected_rows($conn) > 0) {echo "<script>alert('data berhasil dirubah');
      document.location.href = 'guru_db.php';</script>";}else{echo"<script>alert('Tidak ada perubahan data'); document.location.href = 'guru_db.php';</script>";}
}
?>
<!doctype html>
<html lang="en">
            ...
            <div class="row">
              <div class="col-md-8">
                <div class="form-group">
                   ...
                   <input type="text" name="guru" value="<?=$row["guru"]; ?>" class="form-control" id="guru" disabled >
                </div>
              </div>
            <div class="col-md-2">
              <div class="form-group">
