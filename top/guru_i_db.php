...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM guru WHERE switch = 'off' AND code LIKE '%$keyword%' OR guru LIKE '%$keyword%' OR jabatan LIKE  '%$keyword%'OR pelajaran LIKE '%$keyword%'ORDER BY code asc");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM guru WHERE switch = 'off'");
      $sql3 = mysqli_num_rows($sq);
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM guru WHERE switch = 'off' ORDER BY code asc LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM guru WHERE switch = 'off'");
      $sql3 = mysqli_num_rows($sq); }
  ...
  <body>
   ...
   <!-- -------------- -->
    <div class="form p-1">
      ...
      <span style="font-size: 20px">Database Guru Inbox [<a href="guru_db.php">Reset</a>]</span>
        <span class="hidden float-right"><a href="guru_i_db_cetak.php" class="btn btn-outline-info btn-sm"><img src="../img/print.png"></a></span>
        <span class="hidden float-right"><a href="guru_i_add.php" class="btn btn-outline-info btn-sm"><img src="../img/add.png"></a></span> 
      <span class="float-right">
       ...
       <table class="table table-hover">
          ...
          <tr>
            <th><?=$no;?></th>
            <td class="d-inline-block align-top" style="text-align: center;"><img src="../img/guru/<?=$row["foto"];?>"width ="50px"></td>
            <td><?=$row["guru"];?></td>
            ...
            <td class="text-center na"><a href="guru_i_edit.php?id=<?=$row["id_guru"]; ?>">O</a> </td>
          </tr>
        <?php $no++;?>
         <?php endwhile; ?>
      </table>
