...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM keluar WHERE guru = 'Guru' AND tanggal LIKE '%$keyword%' OR  des LIKE '%$keyword%' ORDER BY id_keluar DESC");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM keluar");
      $sql3 = $sql2;
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM keluar WHERE guru = 'Guru' ORDER BY id_keluar DESC LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM keluar ");
      $sql3 = mysqli_num_rows($sq);   
    }
    $akhir = ceil($sql3/$halaman);
    $query = "UPDATE guru SET info = 'outline-primary' WHERE id_guru = '$id_guru'";
