...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM guru WHERE status != 'Inbox' AND code LIKE '%$keyword%' OR guru LIKE '%$keyword%' OR jabatan LIKE  '%$keyword%'OR pelajaran LIKE '%$keyword%'ORDER BY id_guru asc");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM guru WHERE status != 'Inbox'");
      $sql3 = mysqli_num_rows($sq);
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM guru WHERE status != 'Inbox' ORDER BY id_guru asc LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM guru WHERE status != 'Inbox'");
      $sql3 = mysqli_num_rows($sq);   
    }
