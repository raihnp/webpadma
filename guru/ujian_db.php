...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM kalender WHERE user = '$guru' AND date LIKE '%$keyword%' OR kls LIKE '%$keyword%'ORDER BY id_kalender DESC");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM kalender WHERE user = '$guru'");
      $sql3 = $sql2;
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM kalender WHERE user = '$guru' ORDER BY id_kalender DESC LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM kalender WHERE user = '$guru'");
  ...
  <body>
   <div class="container">
    ...
    <div class="card p-1" style="width: 100%;">
      ...
      <span style="font-size: 20px">UjianDb</span>
  ...
  <table class="table table-hover">
       ...
       <?php $date = $row["date"]; 
         ...
         ?>
          <tr>
            <td><?= $s; ?></td>
            <td class="hidden"><?= $tgl; ?><br><i><?=$row["kls"]; ?></i></td>
            <td class="hidden"><?=$row["description"]; ?><br><?=$row["keterangan"]; ?></td>
            <td class="hidden text-center"><a href="ujian_edit.php?id=<?=$row["id_kalender"]; ?>">O</a></td>  
          </tr>
            <?php $s++; ?>
        <?php endwhile; ?>  
        </table> 
