...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM siswa WHERE switch = 'off' AND code LIKE '%$keyword%' OR siswa LIKE '%$keyword%' OR kls LIKE  '%$keyword%' ORDER BY code asc");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM siswa WHERE switch = 'off'");
      $sql3 = mysqli_num_rows($sq);
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM siswa WHERE switch = 'off' ORDER BY code asc LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM siswa WHERE switch = 'off'");
      $sql3 = mysqli_num_rows($sq); }
  ...
  <body>
   ...
   <!-- -------------- -->
    <div class="form p-1">
      ...
      <span style="font-size: 20px">Database Siswa Inbox [<a href="siswa_db.php">Reset</a>]</span>
        <span class="hidden float-right"><a href="siswa_i_db_cetak.php" class="btn btn-outline-info btn-sm"><img src="../img/print.png"></a></span>
        <span class="hidden float-right"><a href="siswa_i_add.php" class="btn btn-outline-info btn-sm"><img src="../img/add.png"></a></span> 
      <span class="float-right">
       ...
       <table class="table table-hover">
          ...
          <tr>
            <th><?=$no;?></th>
            <td class="d-inline-block align-top" style="text-align: center;"><img src="../img/siswa/<?=$row["foto"];?>"width ="50px"></td>
            <td><?=$row["siswa"];?><br><i>Terakhir : <?=$row["kls"];?></i></td>
            ...
            <td class="text-center na"><a href="siswa_i_edit.php?id=<?=$row["id_siswa"]; ?>">O</a><br><a href="ortu_cab.php?nis=<?=$row["nis"]; ?>&nama=<?=$row["siswa"]; ?>&kls=$k; ?>">Ortu</a></td>
          </tr>
        <?php $no++;?>
         <?php endwhile; ?>
      </table>
