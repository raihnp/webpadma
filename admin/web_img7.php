...
$query = mysqli_query($conn,"SELECT img7, vid7 FROM web");
...
if(isset($_POST["submit"])){
  ...
  $filefoto = $_FILES["filefoto"]["name"];
   ...
   $query = "UPDATE web SET img7 = '$foto', vid7 = '$vid7' WHERE id_web ='1'";
    ...
    <div class="form p-1">
      ...
      <form method="POST" action="" enctype="multipart/form-data">
          <div class="row mt-2 pl-2">
            <div class="col-md-4">
                <img src="../img/web/<?=$row["img7"]; ?>" width = "200px"><br><i>Terpasang</i>
            </div>
            <div class="col-md-8">
              <div class="form-group">
                <i>Ganti Gambar</i>
                 <input type="file" name="filefoto" value="" class="form-control" id="filefoto">
                  <input type="hidden" name="fotolama" value="<?=$row["img7"]; ?>">
              </div>
            </div>
          </div>
        <div class="row">
          <div class="col-md-2">
              <?php 
                ...
                <?php }else{
                   if(preg_match("/youtube/", $row['vid7'])) {;?>
                      <div class="embed-responsive embed-responsive-16by9">
                        <?=$row['vid7'];?>
                      </div>
                    <?php } else {echo $row['vid7'];}
                } ?>
            </div>
          ...
          <div class="form-group">
            <i>Ganti Video</i>
             <div class="form-group">
              <textarea  name="vid7" id="ket" class="form-control" ><?=$row["vid7"]; ?></textarea>
            </div>
          </div>
        </div>
        </div>
