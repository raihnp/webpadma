...
$query = mysqli_query($conn,"SELECT img3, vid3 FROM web");
...
if(isset($_POST["submit"])){
  ...
  $filefoto = $_FILES["filefoto"]["name"];
   ...
   $query = "UPDATE web SET img3 = '$foto', vid3 = '$vid3' WHERE id_web ='1'";
    ...
    <div class="form p-1">
      ...
      <form method="POST" action="" enctype="multipart/form-data">
          <div class="row mt-2 pl-2">
            <div class="col-md-4">
                <img src="../img/web/<?=$row["img3"]; ?>" width = "200px"><br><i>Terpasang</i>
            </div>
            <div class="col-md-8">
              <div class="form-group">
                <i>Ganti</i>
                 <input type="file" name="filefoto" value="" class="form-control" id="filefoto">
                  <input type="hidden" name="fotolama" value="<?=$row["img3"]; ?>">
              </div>
            </div>
          </div>
        <div class="row">
          <div class="col-md-2">
              <?php 
                ...
                <?php }else{
                   if(preg_match("/youtube/", $row['vid3'])) {;?>
                      <div class="embed-responsive embed-responsive-16by9">
                        <?=$row['vid3'];?>
                      </div>
                    <?php } else {echo $row['vid3'];}
                } ?>
            </div>
          ...
          <div class="form-group">
            <i>Ganti Video</i>
            <div class="form-group">
              <textarea  name="vid3" id="ket" class="form-control" ><?=$row["vid3"]; ?></textarea>
            </div>
          </div>
          Untuk video dari facebook gunakan embed hasil <a href="https://developers.facebook.com/docs/plugins/embedded-video-player#configurator">generator</a>
        </div>
        </div>
      </div>
