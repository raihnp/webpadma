...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM kalender WHERE title != 'Padmanaba' AND user = '$gurux' AND date LIKE '%$keyword%' OR kls LIKE '%$keyword%'ORDER BY id_kalender DESC");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM kalender WHERE title != 'Padmanaba' AND user = '$gurux'");
      $sql3 = $sql2;
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM kalender WHERE title != 'Padmanaba' AND user = '$gurux' ORDER BY id_kalender DESC LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM kalender WHERE title != 'Padmanaba' AND user = '$gurux'");
...
$query = "UPDATE guru SET ujian = 'outline-success' WHERE id_guru = '$id_guru'";
...
<div class="col-md-10 mt-3">
    ...
    <div class="card p-1" style="width: 100%;">
      ...
      <span style="font-size: 20px">Ujian</span>
       ...
       <form action="" method="POST">
               <div class="input-group input-group-sm">
                  <input type="text" class="form-control" placeholder="Pencarian..." name="keyword" autocomplete="off">
                    ...
                    <span class="hidden float-right"><a href="ujian_add.php" class="btn btn-outline-info btn-sm"><img src="../img/add.png"></a></span>
                </div>    
          </form>
          </span>    
     </div>
  <table class="table table-hover">
        ...
        <?php while($row = mysqli_fetch_assoc($sql)) : ?>
          ...
          <tr>
            ...
            <td><?=$row["description"]; ?><br><i><?=$row["keterangan"]; ?></i></td>
            <td></td>
            <td><a href="Ujian_edit.php?id=<?=$row["id_kalender"]; ?>">O</a> | <a href="ujian_delete.php?id=<?=$row["id_kalender"]; ?>"onclick = "return confirm('yakin ?');">X</a></td>  
          </tr>
        <?php endwhile; ?>  
        </table> 
