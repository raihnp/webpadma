...
$sqlx = mysqli_query($conn, "SELECT * FROM guru WHERE id_guru = '$id_guru' ORDER BY code asc");
...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM guru WHERE switch = 'on' AND kodeg LIKE '%$keyword%' OR guru LIKE '%$keyword%' OR jabatan LIKE  '%$keyword%'OR pelajaran LIKE '%$keyword%'ORDER BY kodeg asc");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM guru WHERE switch = 'on'");
      $sql3 = mysqli_num_rows($sq);
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM guru WHERE switch = 'on'  ORDER BY kodeg asc LIMIT $mulai,$halaman"); 
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM guru WHERE switch = 'on'");
      $sql3 = mysqli_num_rows($sq);   
    }
  ...
  <body>
   ...
   <!-- -------------- -->
    <div class="form p-1">
      ...
      <span style="font-size: 20px">Database Guru [<a href="guru_i_db.php">Inbox</a>]</span>
        <span class="hidden float-right"><a href="guru_db_cetak.php" class="btn btn-outline-info btn-sm"><img src="../img/print.png"></a></span>
      ...
      <span class="float-right">
       ...
       <table class="table table-hover">
          ...
          <tr>
            <th><?=$no;?></th>
            <td class="d-inline-block align-top" style="text-align: center;"><img src="../img/guru/<?=$row["foto"];?>"width ="50px"><br><?=$row["kodeg"];?> | <?=$row["code"];?></td>
            <td><?=$row["guru"];?><br><i><?=$row["jabatan"];?><br><?=$row["pelajaran"];?></i></td>
            ...
            <td class="text-center na"><a href="guru_edit.php?id=<?=$row["id_guru"]; ?>">O</a> | <a href="guru_delete.php?id=<?=$row["id_guru"]; ?>" onclick = "return confirm('yakin ?');">X</a><br><?=$row["status"]; ?></td>
          </tr>
        <?php $no++;?>
         <?php endwhile; ?>
      </table>
