...
$id =$_GET["id"];
$tug = mysqli_query($conn, "SELECT * FROM tugas WHERE id_tugas = '$id'");
$tu = mysqli_fetch_assoc($tug);
...
if(isset($_POST["submit"])){
  ...
  $keterangan = $_POST["keterangan"];
     $query = "UPDATE tugas SET guru = '$guru', pelajaran = '$pelajaran', tanggal = '$tanggal', kls = '$kls', judul = '$judul', keterangan = '$keterangan' WHERE id_tugas = '$id'";
     mysqli_query($conn, $query);
        ...
        $query1 = "UPDATE siswa1, siswa2 SET siswa2.tugas = 'success' WHERE siswa1.id_siswa = siswa2.id_siswa AND siswa1.kls like '%$kls%'";
        mysqli_query($conn, $query1);

        $que = mysqli_query($conn, "UPDATE guru SET tugas_s = 'success' WHERE id_guru = '$id_guru'");
...
<div class="col-md-10 mt-3">
    ...
    <form action="" method="POST">
      <div class="row">
        ...
        <input type="date" name="tanggal" class="form-control" id="date" value="<?=$tu["tanggal"] ;?>" required>
      </div>
    </div>
      <div class="col-md-2">
        <div class="form-group">
...
<div class="row">
      ...
      <div class="form-group">
        ...
        <input type="text" name="judul" class="form-control" id="judul" value="<?=$tu["judul"] ;?>" required>
      </div>
    </div>
  </div>
      <div class="form-group">
        <label for="ket" class="control-label">Keterangan</label>
      <textarea class="ckeditor" name="keterangan" id="ket" class="form-control" ><?=$tu["keterangan"] ;?></textarea>
    </div>
 <hr>     
      <input type="submit" name="submit" class="btn btn-outline-dark" value="Rubah" />
      <a href="tugas_db.php" class="btn btn-outline-dark">Cancel</a>
    </form>
  </div>
   <!-- -------------- -->
</div>
