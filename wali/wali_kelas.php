...
if(isset($_POST["search"])) {
      ...
      $sql = mysqli_query($conn, "SELECT * FROM siswa WHERE switch = 'on' AND  kls LIKE  '%$id%' AND siswa LIKE '%$keyword%' OR nis LIKE '%$keyword%' ORDER BY nis ASC");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM siswa WHERE switch = 'on' AND  kls LIKE  '%$id%'");
      $sql3 = $sql2;
      $no = 1;
  }else{
      $sql = mysqli_query($conn, "SELECT * FROM siswa WHERE switch = 'on' AND kls LIKE  '%$id%' ORDER BY nis ASC");
      $sql2 = mysqli_num_rows($sql);
      $sq= mysqli_query($conn, "SELECT * FROM siswa WHERE switch = 'on' AND  kls LIKE  '%$id%'");
      $sql3 = mysqli_num_rows($sq);
    }
    $akhir = ceil($sql3/$halaman);
$la = mysqli_query($conn, "SELECT * FROM siswa WHERE kls LIKE '%$id%' AND jk = 'Laki-laki'");
$laki = mysqli_num_rows($la);
?>
<!doctype html>
<html lang="en">
  <head>
...
<div class="form p-1">
    <div class="card p-1" style="width: 100%;">
      ...
      <span style="font-size: 20px">Manajemen Siswa <?= $id; ?></span>
      ...
      <p style="color:blue; font-size: 12px"><i>jumlah siswa = <?= $sql3 ;?> | Laki-laki = <?= $laki ;?> | Perempuan = <?= $sql3-$laki ;?></i></p>    
     </div>
 <table class="table table-hover">
    <tr>
      ...
      <th>Foto</th>
      <th>Nama</th>
      <th class="na">Akademik</th>
      <th class="na">Status</th>
      <th class="na"><img src="../img/setting.png"></th>
    </tr>
    ...
    <tr>
      ...
      <td><?=$row["siswa"];?></td>
      <td class="na">
        <a href="naik.php?id=<?=$row["id_siswa"];?>">Naik</a><br><a href="tinggal.php?id=<?=$row["id_siswa"];?>">Tinggal</a><br><a href="pindah.php?id=<?=$row["id_siswa"];?>">Pindah</a></td>
      <td class="na"><a href="siswa_pilihan_edit.php?id=<?=$row["id_siswa"];?>&k=<?=$id;?>">Pilihan</a><br><a href="note_db.php?id=<?=$row["id_siswa"];?>">Catatan</a><br><a href="absen.php?id=<?=$row["id_siswa"];?>">Absensi</a></td>
      <td class="text-center na"><a href="siswa_aktif.php?id=<?=$row["id_siswa"]; ?>&kls=<?=$id;?>"><?=$row["status"]; ?></a><br><a href="siswa_reset.php?id=<?=$row["id_siswa"]; ?>&kls=<?=$id;?>"onclick = "return confirm('yakin ?');">Reset</a><br></td>
    </tr>
   <?php endwhile; ?>
</table>
