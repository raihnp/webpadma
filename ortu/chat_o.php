...
$sqlg = mysqli_query($conn, "SELECT * FROM guru WHERE jabatan = '$jabatan'");
$sqg = mysqli_fetch_assoc($sqlg);
$guru = $sqg["guru"];
$id_guru = $sqg["id_guru"];
$siswa=$sqz["siswa"];
$foto = $rowx["foto"];
$cha = mysqli_query($conn, "SELECT * FROM chat WHERE untuk = ' ' AND (kepada = '$ortu' OR dari = '$ortu')");
$kls = $k;
$dari = $ortu;
$untuk = ' ';
$kepada = $guru;
...
if(isset($_POST["submit"])){
  $jawab = $_POST["chat"];
     ...
     if (mysqli_affected_rows($conn) > 0) {
          ...
          document.location.href = 'chat_o.php';
          </script>
          ";
        } else {
          echo"
