...
$sql = mysqli_query($conn,"SELECT * FROM tugas WHERE kls ='$k' ORDER BY id_tugas DESC");

$query = "UPDATE ortu SET tugas = 'outline-success'";
...
<div class="col-md-10">
    ...
    <div class="card-header">
     <span style="font-size: 20px">Tugas <?=$siswa?></span>
   </div>
       ...
       <?php $tanggal = $row["tanggal"]; 
         ...
         ?>
          <tr>
            <td><?= $s; ?></td>
            <td><?= $tgl; ?></td>
            <td><b><?= $row["pelajaran"] ?></b></td>
            <td class="hidden"><?=$row["judul"]; ?><br><i><?=$row["keterangan"]; ?></i></td>
            <?php $s++; ?>
          </tr>
        <?php endwhile; ?>  
        </table>
